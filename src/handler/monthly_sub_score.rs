...
impl ExecSql<MonthlySubScoreReq> for MonthlySubScore {
    async fn handle_post(
        cfg: Extension<Arc<Config>>,
        prms: Result<Json<MonthlySubScoreReq>, JsonRejection>,
        ...
        match prms.id {
            Some(id) => {
                if prms.state == 0 {
                    let sql = "DELETE FROM monthly_sub_score WHERE id = ?";
